...
app.controller('PortfolioCtrl', function($scope, $http, $location) {

  $http.defaults.headers.post.Authorization = localStorage.getItem("auth");
  //$scope.host = "http://192.168.23.65:9091"; //Junaid
  //$scope.host = "http://192.168.23.1:9091"; //Deployment
  $scope.host = "http://192.168.23.1:9081"; //Testing
  //$scope.host = ""; //Lucky
  //$scope.host = ""; //Kingsley
  
  // Model variables that will be used
  $scope.netAssetValue = {
    'val': 100792.09,
    'percentage': parseInt(Math.random()*100)/10
  };
  
  $scope.netAssetUp = true;
  $scope.portfolioValue = 13137.77;
  $scope.cashAvailable = 87654.32;
  $scope.charts = [
  {
    title: 'Market Value',
    data: []
  ...
  // Data that should be fetched:
  // - Stock list (for the search bar)
  // - 5 market indices (index name, value, change(%))
  // - Portfolio value chart data
  // - Data of 3 tables
  // - User portfolio value info (3 value, 1 percentage+trend)
  // - Chip
  $http.get( $scope.host+"/portfolio/details?username=" + localStorage.getItem("username"), config
    ).success(function(response) {
      // Set the model value of money part
      // $scope.userprofile = response;
      $scope.cashAvailable = response.summary.cash;
      $scope.netAssetValue.val = response.summary.netAssetValue;
      $scope.netAssetValue.percentage = Math.round(response.summary.delta * 10000) / 100;
      $scope.portfolioValue = response.summary.totalPortfolio;
      $scope.netAssetUp = response.summary.delta >= 0;

      // Set the model value of Tables part
      $scope.tables = $scope.ToolMakeTablesModel(response);
  ...
  // Get the data for the market index table
  ...
  //$http.get($scope.host+"/marketdata/get/historical?stock="+stock, config
    ...
    console.error("Error when fetching data for market index table.")
  });
  ...
  // Make the data structure for the tables
  $scope.ToolMakeTablesModel = function(srcData){
    // Parameters:
    // -----------
    // 1. srcData:  response.portfolio

    // Returns:
    // 1. resTables:  Should be assigned to $scope.tables

    // For test only, since no data provided by DS
    if (srcData.portfolio === undefined || srcData === null) {
      srcData.portfolio = [
        {
          "userId": 4,
          "stockId": "TUI",
      ...
      srcData.assetTypeDistribution = {
        "stock": 1.0,
        "index": 0.0
      };
      srcData.assetClassDistribution = {
        "Equity": 1.0,
        "FixedIncome": 0.0,
        "Commodities": 0.0
      };
    ...
    var resTables = [];
    var dictClassToIndex = {};

    if (srcData.portfolio === undefined || srcData.length === 0) {
      console.error('Nothing in your portfolio now.');
      return [];
    }

    var i = 0
    for (name in srcData.assetClassDistribution) {
      resTables.push({
        title: name,
        percentage: srcData.assetClassDistribution[name],
        data:[]
      })
      dictClassToIndex[name] = i++;
    }

    srcData.portfolio.forEach(function(obj, index, array){
      if (obj.asset.assetClass in dictClassToIndex) {
        resIndex = dictClassToIndex[obj.asset.assetClass];
        resTables[resIndex].data.push(obj);
      }
    });
  ...
  $scope.toolRoundValue = function(value) {
    // Change the value to percentage and round it.
